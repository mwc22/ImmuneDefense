...
public class MultiMap2 implements MapModel {
	...
	public MultiMap2(Vector<Path> paths, int teamId, int widthOfImage, int heightOfImage) {
		setPaths(paths);
		m_teamId = teamId;
		m_hashThings = new HashMap<Integer, Displayable>();
		m_creeps = new Vector<Creep>();
		m_towers = new Vector<Tower>();
		...
		m_base = new Base(BASE_HEALTH, getPaths().get(0).getLast());
		m_things.add(m_base);
	...
	public void strikeBase(Creep creep, Player player) {
		...
		m_base.strike(creep.getDamageToBase());
		removeCreep(creep);
	}
